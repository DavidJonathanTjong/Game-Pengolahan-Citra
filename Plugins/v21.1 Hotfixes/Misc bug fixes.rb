...
class Game_Picture
  # Melakukan operasi sprite
  def apply_random_sprite_operation(sprite)
    operations = [:flip_horizontal, :flip_vertical, :grayscale, :thresholding, 
                  :sharpness, :blur, :brightness_adjust, :negative]
    @current_sprite_operation = operations.sample # Pilih operasi secara acak
  
    case @current_sprite_operation
    ...
    when :thresholding
      apply_thresholding(sprite, 128)  # Gunakan nilai threshold 128
    when :sharpness
      apply_sharpness(sprite)
    when :blur
      apply_blur(sprite)
    when :brightness_adjust
      adjust_brightness(sprite, 100)
    when :negative
  ...
  def flip_horizontal(sprite)
    ...
    width.times do |x|
      height.times do |y|
        sprite.bitmap.set_pixel(width - x - 1, y, sprite.bitmap.get_pixel(x, y))
      end
    end
  end
  
  # Manual vertical flip
  def flip_vertical(sprite)
    width = sprite.bitmap.width
    height = sprite.bitmap.height
  
    width.times do |x|
      height.times do |y|
        sprite.bitmap.set_pixel(x, height - y - 1, sprite.bitmap.get_pixel(x, y))
      end
    end
  end
  ...
  def apply_grayscale(sprite)
    ...
    width.times do |x|
      height.times do |y|
        ...
        # Konversi RGB ke Y (luminance)
        luminance = (0.299 * color.red + 0.587 * color.green + 0.144 * color.blue).round
        new_color = Color.new(luminance, luminance, luminance, color.alpha)
        sprite.bitmap.set_pixel(x, y, new_color)
      end
    end
  end

  def ubahgray(duration = 0)
    gray_tone = Tone.new(0, 0, 0, 255)  # Max grayscale value
    start_tone_change(gray_tone, duration)
  end
  
  # Thresholding effect
  def apply_thresholding(sprite, threshold = 128)
    width = sprite.bitmap.width
    height = sprite.bitmap.height
    width.times do |x|
      height.times do |y|
        color = sprite.bitmap.get_pixel(x, y)
        grayscale = (color.red + color.green + color.blue) / 3
        if grayscale < threshold
          new_color = Color.new(0, 0, 0, color.alpha)
        else
          new_color = Color.new(255, 255, 255, color.alpha)
        end
        sprite.bitmap.set_pixel(x, y, new_color)
      end
    end
  ...
  # Konvolusi dengan padding untuk penajaman
  def apply_sharpness(sprite)
    kernel = [[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]]
    apply_convolution(sprite, kernel)
  end
  
  # Konvolusi dengan padding untuk blurring
  def apply_blur(sprite)
    kernel_size = 5  # Ukuran kernel (misalnya 5x5)
    sigma = 1.0      # Standar deviasi untuk Gaussian
    kernel = gaussian_kernel(kernel_size, sigma)
    apply_convolution(sprite, kernel)  # Argumen true untuk hanya piksel berwarna
  end
  
  def gaussian_kernel(size, sigma)
    kernel = Array.new(size) { Array.new(size, 0) }
    total = 0.0
    half_size = size / 2
  
    (0...size).each do |x|
      (0...size).each do |y|
        # Hitung nilai Gaussian
        x_offset = x - half_size
        y_offset = y - half_size
        exponent = -(x_offset**2 + y_offset**2) / (2.0 * sigma**2)
        kernel[x][y] = Math.exp(exponent) / (2.0 * Math::PI * sigma**2)
        total += kernel[x][y]
      end
    end
    ...
    # Normalisasi kernel
    kernel.each do |row|
      row.map! { |value| value / total }
    end
  
    kernel
  end
  
  # Fungsi konvolusi umum dengan padding
  def apply_convolution(sprite, kernel)
    width = sprite.bitmap.width
    height = sprite.bitmap.height
    ...
    width.times do |x|
      height.times do |y|
        ...
        # Terapkan kernel
        (-offset..offset).each do |kx|
          (-offset..offset).each do |ky|
            nx = x + kx
            ny = y + ky
  
            if nx.between?(0, width - 1) && ny.between?(0, height - 1)
              weight = kernel[kx + offset][ky + offset]
              color = sprite.bitmap.get_pixel(nx, ny)
  
              r_total += color.red * weight
              g_total += color.green * weight
              b_total += color.blue * weight
            end
          end
        end
  
        new_color = Color.new(r_total.clamp(0, 255), g_total.clamp(0, 255), b_total.clamp(0, 255))
        sprite.bitmap.set_pixel(x, y, new_color)
      end
    end
  end
  ...
  # Adjust brightness
  def adjust_brightness(sprite, adjustment)
    width = sprite.bitmap.width
    height = sprite.bitmap.height
    width.times do |x|
      height.times do |y|
        color = sprite.bitmap.get_pixel(x, y)
        r = (color.red + adjustment).clamp(0, 255)
        g = (color.green + adjustment).clamp(0, 255)
        b = (color.blue + adjustment).clamp(0, 255)
        sprite.bitmap.set_pixel(x, y, Color.new(r, g, b, color.alpha))
      end
    end
  end
  
  # Negative effect
  def apply_negative(sprite)
    width = sprite.bitmap.width
    height = sprite.bitmap.height
    width.times do |x|
      height.times do |y|
        color = sprite.bitmap.get_pixel(x, y)
        new_color = Color.new(255 - color.red, 255 - color.green, 255 - color.blue, color.alpha)
        sprite.bitmap.set_pixel(x, y, new_color)
      end
    end
  end   
